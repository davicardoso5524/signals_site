...
const grants = readFileSync("supabase/functions/admin-access/index.ts", "utf8");
const grantsMigration = readFileSync("supabase/migrations/202609120005_admin_access_grants.sql", "utf8");
...
assert.match(grantsMigration, /create table public\.admin_access_grants/);
assert.match(grantsMigration, /create policy admin_access_grants_admin_insert/);
assert.match(grants, /ADMIN_REQUIRED/);
assert.match(grants, /action === "me"/);
assert.match(grants, /action === "grant"/);
assert.match(grants, /action === "revoke"/);
assert.match(grants, /action === "list"/);
assert.match(grants, /duration_days/);
assert.match(grants, /ends_at/);
